...
% load the bathymetry
load(fullfile(pwd, 'Lofoten_bathy.mat'));

gfilter = fspecial('gaussian', 16, 4);

im1_id = 'fora2014jd054t143319';
im2_id = 'fora2014jd054t143459';

im1 = read_arr(fullfile(pwd,'raw_images',[im1_id '.arr']));
im1 = flipud(im1);
im1 = (im1-min(min(im1)))*255/(max(max(im1))-min(min(im1)));
run(fullfile(pwd,'raw_images',[im1_id '.m']))
...
im2 = read_arr(fullfile(pwd,'raw_images',[im2_id '.arr']));
im2 = flipud(im2);
im2 = (im2-min(min(im2)))*255/(max(max(im2))-min(min(im2)));
run(fullfile(pwd,'raw_images',[im2_id '.m']))
